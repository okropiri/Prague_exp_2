...
void HVSystemLogin()
{
	...
	con_printf("( SY1527 -> 0,  SY2527 -> 1, SY4527 -> 2, SY5527 -> 3, V65XX -> 4, N14XX -> 5, V8100 -> 6, N568E -> 7, DT55XX - > 8, DT55XXE - > 9, SMARTHV - > A, R6060 - > B): ");
	c = con_getch();
	while( c != '0' && c != '1' && c != '2' && c != '3' && c != '4' && c != '5' && c != '6' && c != '7' && c != '8' && c != '9'&& c != 'A' && c != 'B')
		c = con_getch();
	if (c == '0') sysType= c - '0';
	if (c == '1') sysType= c - '0';
	if (c == '2') sysType= c - '0';
	if (c == '3') sysType= c - '0';
	if (c == '4') sysType= V65XX;
	if (c == '5') sysType= N1470;
	if (c == '6') sysType= V8100;
	if (c == '7') sysType= N568E;
	if (c == '8') sysType= DT55XX;
	if (c == '9') sysType= DT55XXE;
	if (c == 'A') sysType= SMARTHV;
	if (c == 'B') sysType= R6060;
	...
	switch( c )
	{
		...
		case '4':
			...
			break;
		case '5': {
			char comm[30], baud[30], data[30], stop[30], parity[30], bn[30] = "0";
			con_printf("\nCOM Port (e.g. COM1): ");
			con_scanf("%s", comm);
			...
			con_scanf("%s", parity);
			if ((sysType != DT55XXE) && (sysType != SMARTHV)){
				con_printf("\nDaisy Chain Board number : ");
				con_scanf("%s", bn);
			}
	...
	sysType == 0 || sysType == 1 ||
#endif
	sysType == 2 || sysType == 3 || sysType == R6060)
...
/* numero di canali */
	...
	if( tipo == PARAM_TYPE_NUMERIC || tipo == PARAM_TYPE_ONOFF )
	{
