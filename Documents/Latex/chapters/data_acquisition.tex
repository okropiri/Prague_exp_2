...
\end{minipage}
\caption{Representative channel-2 folded RF phase-versus-ToT maps across the position scan, displayed with a three-period RF phase axis. The panels motivate the later gamma--neutron separation strategy: at some positions the compact prompt-like island is visibly separated from the broader neutron-dominated distribution, while in overlap regions the event content must be treated as a statistical mixture rather than by unique hit-by-hit labels. The \SI{4.0}{m} panel uses the completed \texttt{NCAL\_20us\_Pos\_4m\_0001} run.}
